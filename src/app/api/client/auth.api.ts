...
export class AuthApiService {
    ...
    login(email: string, password: string): Observable<IHttpResponse<IUser>> {
        ...
        return this.http.post<IHttpResponse<IUser>>('http://localhost:3000/auth/login', data, { headers: this.reqHeader });
    }
    ...
    register(email: string, password: string, username: string): Observable<IHttpResponse<IUser>> {
        ...
        return this.http.post<IHttpResponse<IUser>>('http://localhost:3000/auth/register', data, { headers: this.reqHeader });
    }
